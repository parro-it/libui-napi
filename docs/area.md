...
function buildSolidBrush(color, alpha) {
    ...
    brush.type = libui.DrawBrush.type.solid;
...
const widget = new libui.UiArea(
    function draw(area, p) {
        ...
        const path = new libui.UiDrawPath(libui.UiDrawPath.fillMode.winding);
        path.addRectangle(p.areaWidth/4, p.areaHeight/4, p.areaWidth/2, p.areaHeight/2);
        path.end();
...
* type
    - `libui.DrawBrush.type.solid` for a solid color
    - `libui.DrawBrush.type.linearGradient` for a linear gradient
    - `libui.DrawBrush.type.radialGradient` for a radial gradient


It's also possible to use the `type` property:

```js
myBrush.type = libui.DrawBrush.type.linearGradient;
```
...
* style:
    - `libui.DrawStrokeParams.lineCap.flat` (default)
    - `libui.DrawStrokeParams.lineCap.round`
    - `libui.DrawStrokeParams.lineCap.square`

It's also possible to use the `cap` property:

```js
myStroke.cap = libui.DrawStrokeParams.lineCap.round;
```
...
* style:
    - `libui.DrawStrokeParams.lineJoin.miter` (default)
    - `libui.DrawStrokeParams.lineJoin.round`
    - `libui.DrawStrokeParams.lineJoin.bevel`

It's also possible to use the `join` property:

```js
myStroke.join = libui.DrawStrokeParams.lineJoin.round;
```
...
* `libui.UiAreaKeyEvent.extKeys.escape`
* `libui.UiAreaKeyEvent.extKeys.insert` (equivalent to "Help" on Apple keyboards)
* `libui.UiAreaKeyEvent.extKeys.delete`
* `libui.UiAreaKeyEvent.extKeys.home`
* `libui.UiAreaKeyEvent.extKeys.end`
* `libui.UiAreaKeyEvent.extKeys.pageUp`
* `libui.UiAreaKeyEvent.extKeys.pageDown`
* `libui.UiAreaKeyEvent.extKeys.up`
* `libui.UiAreaKeyEvent.extKeys.down`
* `libui.UiAreaKeyEvent.extKeys.left`
* `libui.UiAreaKeyEvent.extKeys.right`
* function keys:
    - `libui.UiAreaKeyEvent.extKeys.f1`, ..., `libui.UiAreaKeyEvent.extKeys.f12`
* numpad keys: (independent of Num Lock state)
    - `libui.UiAreaKeyEvent.extKeys.n0`, ..., `libui.UiAreaKeyEvent.extKeys.n9`
    - `libui.UiAreaKeyEvent.extKeys.nDot`
    - `libui.UiAreaKeyEvent.extKeys.nEnter`
    - `libui.UiAreaKeyEvent.extKeys.nAdd`
    - `libui.UiAreaKeyEvent.extKeys.nSubtract`
    - `libui.UiAreaKeyEvent.extKeys.nMultiply`
    - `libui.UiAreaKeyEvent.extKeys.nDivide`

### getModifier()

Returns the pressed  modifier key (0 if not). Values:

* `libui.UiAreaKeyEvent.modifierKeys.ctrl`
* `libui.UiAreaKeyEvent.modifierKeys.alt`
* `libui.UiAreaKeyEvent.modifierKeys.shift`
* `libui.UiAreaKeyEvent.modifierKeys.super`
