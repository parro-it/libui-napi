...
static void test_remove_child_empty_list(napi_env env) {
	struct control_handle *child = get_child(env);
	struct children_list *list = create_children_list();
	napi_value ret = remove_child(env, list, child);
	assert(ret == NULL);
}

static void test_remove_child_one_child_list(napi_env env) {
	struct children_list *list = create_children_list();
	struct control_handle *child = get_child(env);
	add_child(env, list, child);
	remove_child(env, list, child);

	assert(list->head == NULL);
	assert(list->tail == NULL);
}

static void test_remove_child_deref_control(napi_env env) {
...
static void test_remove_nonexistent_child_from_child_list(napi_env env) {

	struct children_list *list = create_children_list();
	struct control_handle *child = get_child(env);
	struct control_handle *child2 = get_child(env);
	add_child(env, list, child);
	remove_child(env, list, child2);
...
void children_list_remove_child_suite(napi_env env) {
	RUN_TEST(test_remove_child_one_child_list);
	RUN_TEST(test_remove_child_empty_list);
	RUN_TEST(test_remove_nonexistent_child_from_child_list);
	...
	RUN_TEST(test_remove_child_deref_control);
}
