...
static void test_add_child_at_return_null_on_success(napi_env env) {

	struct children_list *list = create_children_list();
	struct control_handle *child = get_child(env);
	napi_value ret = add_child_at(env, list, child, 0);
...
static void test_add_two_child_set_head_tail(napi_env env) {
	...
	add_child_at(env, list, child2, 0);
	add_child_at(env, list, child1, 0);
...
static void test_add_middle_child(napi_env env) {
	...
	add_child_at(env, list, child2, 0);
	add_child_at(env, list, child1, 0);
	add_child_at(env, list, child3, 1);
...
void children_list_add_child_at_suite(napi_env env) {
	RUN_TEST(test_add_child_at_return_null_on_success);
