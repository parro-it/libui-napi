...
function openFile() {
	...
	if (filename) {
		readFile(filename, 'utf8', (err, content) => {
			if (err) {
				return libui.UiDialogs.msgBoxError(win, 'Error while reading file',
												   err.stack);
			}
			editor.text = content;
...
function saveFileAs() {
	...
	if (filename) {
		...
		writeFile(currentFileName, editor.text, err => {
			if (err) {
				return libui.UiDialogs.msgBoxError(win, 'Error while writing file',
												   err.stack);
			}
		});
	}
...
menu([
	{
		label: 'File',
		submenu: [
			{label: 'New file', click: newFile}, {label: 'Open', click: openFile},
			// {
			// 	label: 'Close current tab',
			// 	click: () => {}
			// },
			{label: 'Save', click: saveFile}, {label: 'Save as', click: saveFileAs},
			{role: 'quit'}
		]
	},
	{
		label: 'Edit',
		submenu:
			[{label: 'Copy'}, {label: 'Paste'}, {label: 'Cut'}, {label: 'Select all'}]
	},
	{label: 'Help', submenu: [{label: 'Help', click: () => {}}, {role: 'about'}]}
]);
